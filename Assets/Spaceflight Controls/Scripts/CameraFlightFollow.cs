...
public class CameraFlightFollow : MonoBehaviour {
	...
	public AircraftPhysics control; //The PlayerFlightControl script that is in play.
	...
	void FixedUpdate () {
		...
		//transform.position = Vector3.Lerp (transform.position, newPosition, Time.fixedDeltaTime * follow_tightness);
		...
		//transform.rotation = Quaternion.Slerp (transform.rotation, newRotation, Time.fixedDeltaTime * rotation_tightness);
